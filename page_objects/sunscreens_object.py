...
class Sunscreens_Object:
    "Page Object for the Sunscreen's  page"
    ...
    def click_all_sunscreen_button(self):
        "Click all add button for Sunscreen button"
        result_flag =False
    ...
    def check_added_count(self):
        ...
        if(number_of_add_button == int(cart_cnt[0])):
            result_flag=True
            self.conditional_write(result_flag,
            positive='All Sunscreen added to the cart',
            negative='Some sunscreen are not added to the cart',
            level='debug')    
